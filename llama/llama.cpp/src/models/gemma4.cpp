...
// Static helper for 2D slice view from a 3D tensor
...
llm_build_gemma4::llm_build_gemma4(const llama_model & model, const llm_graph_params & params) :
    ...
    for (int il = 0; il < n_layer; ++il) {
        // Compute per-layer dimensions based on SWA type
        const bool is_swa_layer = hparams.swa_layers[il];
        const int64_t n_embd_head = is_swa_layer && hparams.n_embd_head_k_swa != 0
            ? hparams.n_embd_head_k_swa
            : n_embd_head_k;
        const int64_t n_rot_l = is_swa_layer && hparams.n_embd_head_k_swa != 0
            ? hparams.n_embd_head_k_swa
            : n_rot;
        ...
        if (is_moe_layer) {
            ...
            // Handle fused gate_up_exps or use separate gate/up tensors
            // The loader tries to load fused gate_up_exps first, falling back to separate tensors.
            // For graph execution, we need separate tensors, so split if needed.
            ggml_tensor * gate_exps = model.layers[il].ffn_gate_exps;
            ggml_tensor * up_exps = model.layers[il].ffn_up_exps;
            
            if (model.layers[il].ffn_gate_up_exps != nullptr) {
                // Split fused gate_up_exps [n_embd, n_ff_exp * 2, n_expert] 
                // into gate and up views by slicing the second dimension
            ...
            cur_moe = build_moe_ffn(cur_moe,
                    ...
                    LLM_FFN_GELU, true, // type_op, norm_w
                    false,  // scale_w (Gemma4 doesn't use scaling)
                    1.0f,   // w_scale
                    LLAMA_EXPERT_GATING_FUNC_TYPE_SOFTMAX,
                    il, logits); // gating_op, il, probs_in
        ...
        if (inp_per_layer) {
            ...
            ggml_tensor * inp_this_layer = view_2d_slice(ctx0, inp_per_layer, il); // [n_embd_per_layer, n_tokens]
